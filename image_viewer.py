...
class Window(QtGui.QWidget):
    def __init__(self):
        ...
        layout.addWidget(self.edit1, 1, 0, 1, 1)
        layout.addWidget(self.edit2, 2, 0, 1, 2)
    ...
    def eventFilter(self, source, event):
        if (event.type() == QtCore.QEvent.MouseMove and
            ...
            if event.buttons() == QtCore.Qt.NoButton:
                if not self.viewer._photo.pixmap().isNull():
                    point = self.viewer.mapToScene(event.pos())
                    self.edit2.setText('x: %d, y: %d' % (round(point.x()), round(point.y())))

        return QtGui.QWidget.eventFilter(self, source, event)

    def handleOpen(self):
        path = QtGui.QFileDialog.getOpenFileName(
            self, 'Choose Image', self.edit1.text())
        if path:
